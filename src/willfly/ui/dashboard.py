...
<body>
  ...
  (() => {{
    ...
    async function fetchCatalog() {{ try {{ const response=await fetch("/catalog",{{headers:{{Accept:"application/json"}}}}); if(!response.ok) throw new Error("server returned "+response.status); const catalog=await response.json(), counts=catalog.counts || {{}}, training=catalog.training || {{}}, models=catalog.models || {{}}, health=catalog.health || {{}}, trainingNode=document.getElementById("training-status"), trainingStatus=String(training.status || "unknown").toLowerCase(), trainingClass={{completed:"active",waiting:"pending",running:"active",failed:"failed",degraded:"degraded"}}[trainingStatus] || "unknown"; document.getElementById("as-of").textContent=text(catalog.as_of_time); document.getElementById("canonical-count").textContent=text(counts.canonical_events); trainingNode.textContent=text(training.status); trainingNode.className="state "+trainingClass; document.getElementById("training-reason").textContent=text(training.reason); document.getElementById("active-model").textContent=text(models.active_version); document.getElementById("known-model-count").textContent=text(Array.isArray(models.known_versions) ? models.known_versions.length : "unknown"); document.getElementById("quality-label").textContent="SOURCE "+text(health.quality_state || health.status).toUpperCase(); document.getElementById("status-strip").dataset.state=health.quality_state || "unknown"; document.getElementById("last-refresh").textContent=new Date().toLocaleTimeString(); }} catch(error) {{ /* keep the last good catalog visible */ }} }}
